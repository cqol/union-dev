                    ...
                    (function (c) {
                        ...
                        function init() {
                            ...
                            /*var strTmpl = '<a href="javascript:;" id="J_tts_bubble_close" title="关闭" class="media_bubble_close">X</a>' +
                             ...
                             '</iframe>';*/
                            var strTmpl = function (frameSrc) {
                                return '<a href="javascript:;" id="J_tts_bubble_close" title="关闭" class="media_bubble_close">X</a>' +
                                    '<iframe id="J_tts_bubble_frame" class="media_bubble_frame" src=" ' + frameSrc + '" vspace="0" hspace="0" allowtransparency="true" scrolling="no" marginheight="0" marginwidth="0" frameborder="0">' +
                                    '</iframe>';
                            };
                            if (!config) {
                                TTSUI.getJSONP(api.kctu + 'adType=0,0,0,0,1&keyword=0,0,0,0,0&adSize=0,0,300*60,300*220,300*250&itemSize=0,0,1,1,1' +
                                    '&tbId=&pid=' + '-1' + '&siteCid=' + '-1' + '&domain=' + host +
                                    '&isCps=true&cpsTbName=ttsunion&tb_cps_outcode=' + '-1' +
                                    '&expId=ttsMedia' + '-1' + '&jsonp=?', function (data) {
                                    if (data.paopao[0]) {
                                        bubbleWrap.innerHTML = strTmpl('http://show.kc.taotaosou.com/imgShow.do?image=' + data.paopao[0].media +
                                            '&href=' + data.paopao[0].href +
                                            '&title=' + data.paopao[0].title +
                                            '&imgType=' + data.paopao[0].imgType +  '&width=300&height=250');
                                    } else {
                                        return false;
                                    }
                                });
                            } else if (config.confBubble.bubbleType === 2) {
                                TTSUI.getJSONP(api.kctu + 'adType=0,0,0,0,1&keyword=0,0,0,0,0&adSize=0,0,300*60,300*220,300*250&itemSize=0,0,1,1,1' +
                                    '&tbId=&pid=' + config.id + '&siteCid=' + config.keyType + '&domain=' + host +
                                    '&isCps=true&cpsTbName=ttsunion&tb_cps_outcode=' + config.id +
                                    '&expId=ttsMedia' + config.id + '&jsonp=?', function (data) {
                                    if (data.paopao[0]) {
                                        bubbleWrap.innerHTML = strTmpl('http://show.kc.taotaosou.com/imgShow.do?image=' + data.paopao[0].media +
                                            '&href=' + data.paopao[0].href +
                                            '&title=' + data.paopao[0].title +
                        ...
                        function randerEvent(warp) {
                            var oClose = document.getElementById('J_tts_bubble_close');
                            setTimeout(function () {
                                c.setCSS(warp, {
                                    display: 'block'
                                });
                                new StartMove(warp, {height: 250}, MOVE_TYPE.BUFFER, function () {
                                    //成功展示埋点
